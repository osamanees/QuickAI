...
const Sidebar = ({ sidebar, setSidebar }) => {
  ...
  return (
    ...
    >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `px-3.5 py-2.5 flex items-center gap-3 rounded ${
                  ...
                  {({isActive})=>(
                    <>
                    <Icon className={`-4 h-4 ${isActive ? 'text-white' : ''}`}/>
                    {label}
                    </>
                  )}
      ...
      justify-between'>
       <div onClick={openUserProfile} className='flex gap-2 items-cente cursor-pointer'>
        ...
        hover:text-gray-700 transition curor-pointer'/>
      </div>
    </div>
  )
}
