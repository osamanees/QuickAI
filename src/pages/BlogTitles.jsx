...
const BlogTitles = () => {
  ...
  const OnSubmitHandler = async (e) => {
    ...
    try{
      setLoading(true)
      const prompt = `Generate a blog title for keyword ${input} in the
    ...
    <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      <form onSubmit={OnSubmitHandler} className="w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200">
            ...
            <span
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer transition 
                ${selectedCategory === item
                  ? 'bg-blue-50 text-purple-700 border-purple-400'
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2
          ...
          {loading ? <span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span> : <Hash className='w-5'/>  }
          Generate title
        </button>
      </form>
